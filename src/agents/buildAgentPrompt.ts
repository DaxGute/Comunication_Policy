...
function protocolLayer(): string {
  return [
    ...
    "Include readyToFinalize: true or false on every Moral and Hidden Profile turn. This is hidden protocol metadata — do not discuss the flag in message.",
...
function reasoningLayer(): string {
  return [
    ...
    "A subject is one independently revisable unit of reasoning state. Crossword: a clue. Moral: a consideration. Hidden Profile: an agent-created decision-relevant consideration (not a seeded option/evidence lane).",
    ...
    "Hidden Profile lanes are considerations only: independently revisable factors, assessments, tradeoffs, or intermediate conclusions about the decision.",
    "The shared Hidden Profile graph starts empty. Agents create considerations with SET; they are never seeded from options or evaluator metadata.",
